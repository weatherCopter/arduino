...
EDIT: 	20150207 Nathan Tyler Rose
		comment out lines 796 - 806 to fix SoftwareSerial SPI12 conflict.
...
3.	Constructor, Destructor, SDI12_NTR.begin(), and SDI12_NTR.end() 
...
#include <SDI12_NTR.h>						// 0.1 header file for this library
...
SDI12_NTR *SDI12_NTR::_activeObject = NULL;		// 0.9 pointer to active SDI12_NTR object
...
void SDI12_NTR::setState(uint8_t state){
  if(state == HOLDING){
...
//	3.1	Constructor
SDI12_NTR::SDI12_NTR(uint8_t dataPin){ _bufferOverflow = false; _dataPin = dataPin; }   

//	3.2	Destrutor
SDI12_NTR::~SDI12_NTR(){ setState(DISABLED); }

//  3.3 Begin
void SDI12_NTR::begin() { setState(HOLDING); setActive(); }   

//  3.4 End
void SDI12_NTR::end() { setState(DISABLED); }
...
void SDI12_NTR::writeChar(uint8_t out)
{
...
void SDI12_NTR::sendCommand(String cmd){
...
pattern: 
			...
			mySDI12_NTR.forceHold(); 
			myOtherSDI12_NTR.setActive(); 

Other notes: Promoting an object into the Active state will set it as
HOLDING. See 6.1 for more information.

Calling mySDI12_NTR.begin() will assert mySDI12_NTR as the new active object,
until another instance calls myOtherSDI12_NTR.begin() or
myOtherSDI12_NTR.setActive().

Calling mySDI12_NTR.end() does NOT hand-off active status to another SDI-12
instance.

You can check on the active object by calling mySDI12_NTR.isActive(), which
...
bool SDI12_NTR::isActive() { return this == _activeObject; }
...
ISR(PCINT0_vect){ SDI12_NTR::handleInterrupt(); }
#endif

//#if defined(PCINT1_vect)
//ISR(PCINT1_vect){ SDI12_NTR::handleInterrupt(); }
//#endif

//#if defined(PCINT2_vect)
//ISR(PCINT2_vect){ SDI12_NTR::handleInterrupt(); }
//#endif

//#if defined(PCINT3_vect)
//ISR(PCINT3_vect){ SDI12_NTR::handleInterrupt(); }
//#endif
