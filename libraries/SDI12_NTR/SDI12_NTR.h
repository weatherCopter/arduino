...
{
private:
  static SDI12_NTR *_activeObject;	// static pointer to active SDI12_NTR instance
  void setState(uint8_t state); // sets the state of the SDI12_NTR objects
  void wakeSensors();			// used to wake up the SDI12_NTR bus
...
public:
  SDI12_NTR(uint8_t dataPin);		// constructor
  ~SDI12_NTR();						// destructor
